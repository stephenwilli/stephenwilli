...
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./utils */ "./assets/src/utils.js");
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()(window).on('load', function () {
  ...
  if (grid2) {
    var iso = new isotope_layout__WEBPACK_IMPORTED_MODULE_3___default.a(grid2, {
...
/*!*****************************!*\
  !*** ./assets/src/utils.js ***!
  \*****************************/
/*! exports provided: noop, isArray, toArray, toBoolean, isBoolean, isElement, isEmpty, isDefined, isFunction, isNumber, isNumeric, isObject, isString, isUndefined, findAll, findOne, on, off, h, html, svg, lockScroll, clamp, wait, request */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
function isObject(value) {
  return _babel_runtime_helpers_typeof__WEBPACK_IMPORTED_MODULE_1___default()(value) === 'object';
}
;
...
function on(type, selector, fn) {
  ...
  var useCapture = scope === document;

  var listener = function listener(event) {
    var target = event.target !== document && event.target.closest(selector);
...
function off(type, selector, fn) {
  var scope = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : document;
  var listeners = on.listeners || [];
  var index = listeners.findIndex(function (o) {
    return o.type === type && o.selector === selector && o.fn === fn && o.scope === scope;
  });

  if (index !== -1) {
    var context = listeners.splice(index, 1)[0];
...
function h(tag, props) {
  ...
  for (var _i = 0; _i < children.length; _i++) {
    var child = isString(children[_i]) ? h(children[_i]) : children[_i];
...
function html(strings) {
  ...
  for (var i = 0; i < args.length; i++) {
    if (args[i] instanceof HTMLElement) {
      var id = "id".concat(i);
      appends[id] = args[i];
      result += "".concat(strings[i], "<div append=\"").concat(id, "\"></div>");
    } else {
  ...
  _babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_0___default()(content.querySelectorAll('[append]')).forEach(function (refNode) {
    var newNode = appends[refNode.getAttribute('append')];
...
function svg(strings) {
  ...
  var string = strings.map(function (str, i) {
    return "".concat(str).concat(vars[i] || '');
  }).join('');
  var parser = svg.parser || (svg.parser = new DOMParser());
  return parser.parseFromString(string, 'image/svg+xml').documentElement;
}
;
var lockScroll = function () {
  var html = document.documentElement; // const body = document.body;

  var isLocked = false;
  return function () {
    if (isLocked) return noop;
    isLocked = true;
    var scrollY = window.scrollY || window.pageYOffset || html.scrollTop || 0;
    html.classList.add('no-overflow'); // body.classList.add('no-overflow');

    return function unlockScroll() {
      isLocked = false;
      html.classList.remove('no-overflow'); // body.classList.remove('no-overflow');
...
function request(method, url, data) {
  ...
  if (!url.startsWith('http')) {
    url = "/".concat(url.startsWith('/') ? url.slice(1) : url);
  }

  if (method.toUpperCase() === 'GET') {
    var params = new URLSearchParams(data).toString();
    params && (url += "".concat(url.includes('&') ? '&' : '?').concat(params));
  } else {
    config.body = JSON.stringify(data || {});
  }

  return window.fetch(url, config).then(function (res) {
...
['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].forEach(function (method) {
  request[method.toLocaleLowerCase()] = function () {
