...
class TMBR_Weather {
	...
	public function add_actions()
	{
		...
		add_action( 'init', function() {
			TMBR_Weather::$lat = apply_filters( 'tmbr/weather/latitude', '43.4799291' );
			TMBR_Weather::$lng = apply_filters( 'tmbr/weather/longitude', '-110.76242819999999' );
	...
	public function get_full_weather()
	{
		...
		if ( $weather_obj === false || !self::$use_cache )
		{
			...
			if ( self::is_valid_response( $r ) )
			{
				...
				if ( $weather_obj )
				{
					set_transient( $transient_key, $weather_obj, 30 * 60 ); // .5 hour cache
				}
			}
			else
			{
